...
class CFormElement implements ArrayAccess {
  ...
  public function Validate($rules, $form) {
    ...
    $tests = array(
      ...
      'pass'          => array('message' => 'Kommer alltid att lyckas.',                    'test' => 'return true;'),
      'not_empty'     => array('message' => 'Får inte vara tomt.',                          'test' => 'return $value != "";'),
      'not_equal'     => array('message' => 'Värdet är felaktigt.',                         'test' => 'return $value != $arg;'),
      'numeric'       => array('message' => 'Måste innehålla enbart siffror.',              'test' => 'return is_numeric($value);'),
      ...
      'three_digits'    => array('message' => 'Måste innehålla exakt 3 siffror.',   'test' => function($value) { return preg_match('/\b[0-9]{3}\b/i', $value) === 1; } ),
      'custom_test'   => true,
    );
